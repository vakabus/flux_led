...
class  NetworkScanner(object):
    ...
    def get_broadcast_ips():
        f = os.popen('ip a | grep inet | grep brd | sed "s/.*brd //;s/ scope.*//"')
        s = f.read().split('\n')
        return [x for x in s if x != '']

    def getLightInfo(self):
        return self.found_lights    
    ...
    def scan(self, timeout=10):
        ...
        broadcast_ips = NetworkScanner.get_broadcast_ips()
        ...
        while True:
            ...
            # send out a broadcast query to every network
            for ip in broadcast_ips:
                sock.sendto(MSG, (ip, DISCOVERY_PORT))
        ...
        self.found_lights = response_list
...
def parseArgs():
    ...
    op_selected = False
    op_selected = op_selected or options.on
    op_selected = op_selected or options.off
    op_selected = op_selected or options.info
    op_selected = op_selected or options.getclock
    op_selected = op_selected or options.setclock
    op_selected = op_selected or options.listpresets
    op_selected = op_selected or options.settimer
    
    if (not options.scan or options.scanresults) and op_selected:
...
def main():
    ...
    if options.scan:
        ...
        bulb_info_list = scanner.getLightInfo()
        ...
        if options.scanresults and len(bulb_info_list) > 0 :
            ...
            print("{} lights found".format(len(bulb_info_list)))
            for b in bulb_info_list:
